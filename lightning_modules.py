...
from lib.models import GINNet, GCNNet, build_mlp, FinalLayers, GNN, RPGNN, PNANet, DeepSet
from lib.subgraph_models import KaryGNN, KaryRPGNN, GraphletCounting
from losses import Loss, IRMLoss, CELoss, SubgraphRegularizedLoss, LabelSmoothingLoss, CentralMomentDiscrepancyLoss
...
class AbstractLightningModule(pl.LightningModule):
    def __init__(self, conf: Config, loss: Loss, **kwargs):
        ...
        os.environ["PYTHONHASHSEED"] = str(self.conf.seed)
        torch.manual_seed(self.conf.seed)
        torch.cuda.manual_seed(self.conf.seed)
        ...
        if not (conf.model == ModelName.GraphletCounting and conf.num_layers == 1):
            ...
            model = FinalLayers(
                ...
                unc_weight=conf.unc_weight,
                feat_aggr_net=DeepSet(self.train_ds.num_node_features,3,2,nn.ReLU) if conf.coarse_pool == "deepset" else None
            )
    ...
    def on_train_epoch_start(self):
        ...
        if self.conf.deepset_alternate_training.presence:
            if self.current_epoch == 0:
                if self.conf.deepset_alternate_training.deepset_first:
                    self.ds_training = True
                    for param in self.model.parameters():
                        param.requires_grad = False
                    for param in self.model.feat_aggr_net.parameters():
                        param.requires_grad = True
                else:
                    self.ds_training = False
                    for param in self.model.feat_aggr_net.parameters():
                        param.requires_grad = False
            elif self.current_epoch % self.conf.deepset_alternate_training.period_length == 0:
                if self.ds_training:
                    self.ds_training = False
                    for param in self.model.parameters():
                        param.requires_grad = True
    ...
    def validation_step(self, batch: Union[Batch, PyGBatch], batch_idx: int,
        ...
        if isinstance(loss, dict):
            if self.conf.only_ce_for_early_stopping:
                loss = loss["ce"]
            else:
                loss = loss["tot"]
    ...
    def validation_epoch_end(self, val_outputs) -> None:
        ...
        val_loss_mean = sum([o['val/loss'] for o in val_outputs]) / len(val_outputs)
        self.log('val/loss', val_loss_mean, prog_bar=True)
    ...
    def _load_model_state(cls, checkpoint: Dict[str, Any], strict: bool = True, **cls_kwargs_new):
        ...
        return super()._load_model_state(checkpoint, strict, conf=conf, loss=checkpoint["loss"]).model ## for analyze_embeddings
    ...
    def build_gnn(cls, conf: Config, in_dim_or_pre_mlp: Union[int, nn.Sequential], deg=None, act=nn.ReLU):
        if conf.gnn_type == "gin":
            gnn = GINNet(
                ...
                act=act,
                jk=conf.jk,
                batch_norm=conf.gnn_batch_norm,
                normalize_emb=conf.normalize_emb
            )
...
def build_trainer_and_model(conf, dirpath='.', progress=False) -> Tuple[pl.Trainer, AbstractLightningModule]:
    ...
    if conf.irm is not None:
        ...
        model = cons(conf, CentralMomentDiscrepancyLoss(conf.dataset_name,
                                                        coarse_ratios=conf.coarse_ratios,
                                                        fine_grained=conf.fine_grained_cmd, 
                                                        loss_computation_mode=conf.cmd_loss_computation_mode,
                                                        cmd_coeff=conf.cmd_coeff, 
                                                        unc_weight=conf.unc_weight,
                                                        coarse_pool=conf.coarse_pool,
                                                        weighted_ce=conf.weighted_ce,
                                                        graph_level_cmd=conf.graph_level_cmd_reg))
    else:
        model = cons(conf, CELoss(conf.dataset_name))

    kwgs = {}
    if len(model.val_ds_list) > 0:
        if conf.early_stopping_metric == "val/accuracy":
            kwgs["monitor"] = 'val/accuracy'
            kwgs["mode"] = 'max'
        elif conf.early_stopping_metric == "val/loss":
            kwgs["monitor"] = 'val/loss'
            kwgs["mode"] = 'min'
    else:
    ...
    trainer = pl.Trainer(weights_summary='full' if conf.exp_iteration == 0 else None,
                         max_epochs=conf.num_epochs,
                         callbacks=[chkp],
                         deterministic=True, #gradient_clip_val=0.1,
...
def train_and_test(config: Config, dirpath='.', progress=False):
    ...
    train_testing = pl.Trainer(logger=False, gpus=config.num_gpus, checkpoint_callback=False,
                               progress_bar_refresh_rate=0 if not progress else 1)
    ...
    if trainer.checkpoint_callback.best_model_score is not None:
        if conf.early_stopping_metric == "val/accuracy":
            ...
            val_score = train_testing.test(trainer.model, test_dataloaders=trainer.model.val_dataloader(), ckpt_path='best')           
            val_metric = val_score[0]['test/accuracy']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        config_file = os.path.join("configs", sys.argv[1]+".yaml")
    ...
    for k, v in conf.items():
        if isinstance(v, dict):
            ...
            v = "\n  - " + "\n  - ".join([f"{v2}" for v2 in v])     
        print(f"{k}: {v}")

    conf: Config = dacite.from_dict(data_class=Config, data=conf,
                                    config=dacite_conf)

    if not torch.cuda.is_available() and conf.num_gpus > 0:
        conf.num_gpus = 0

    #conf.num_epochs = 19

    num_repeats = conf.num_repeat_exp
    tot_results = {"train": [], "val": [], "test": []}
    start_seed = conf.seed
    for i in range(num_repeats):
        print(f"\n\nIteration: {i}")
        conf.exp_iteration = i
        conf.seed = start_seed + i
        out_path = os.path.join(conf.output_dir, f"{conf.model}_{conf.gnn_type}_{conf.graph_pooling}_{conf.dataset_name}_seed{conf.seed}")
        progress = True if num_repeats == 1 else False
        results = train_and_test(conf, dirpath=out_path, progress=progress)
        print(results)
        for k in tot_results:
            tot_results[k].append(results[k])
    if num_repeats > 1:
        print(f"Results averaged over {num_repeats} seeds:")
        for k in tot_results:
            arr = np.array(tot_results[k])
            print("{}: {} +- {}".format(k, arr.mean(), arr.std()))
