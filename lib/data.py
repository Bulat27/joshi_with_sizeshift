...
from lib.coarsening_utils import get_preproc_coarsening_fun, add_coarsened_edge_index
...
def nearest_neighbor_graph(nodes, neighbors, knn_strat):
    ...
    if neighbors >= num_nodes - 1 or neighbors == -1:
        ...
        # knns = np.argpartition(W_val, kth=neighbors, axis=-1)[:, :neighbors]
    ...
    # Convert the negative adjacency matrix to edge_index
    edge_index = dense_to_sparse(1 - W)[0]
    return edge_index

def add_edge_index_mine(g):
...
# def process_graph(i, g, dataset, method, coarse_ratios, test_idxs, processed_dir):
#     """Process a single graph and return the coarsened graph."""
    
#     print(f"Processing graph {i}")  # Print the graph being processed
#     processed_graph_file_path = osp.join(processed_dir, f"{i}_{method}")
...
#             coarse_ratio_postfix = str(int(ratio * 100))
#             if not hasattr(new_g, f"num_coarse_nodes_{coarse_ratio_postfix}"):
...
# def add_new_ratios(dataset, method, coarse_ratios, test_idxs=None, num_processes=8):
#     """Parallelize graph coarsening using ProcessPoolExecutor with a specified number of processes."""
#     processed_dir = osp.dirname(dataset.processed_paths[0])
    
#     print("Generating coarsened graphs")
#     datalist = []
    
#     # Ensure num_processes is positive
#     if num_processes <= 0:
#         raise ValueError("Number of processes must be greater than 0")
    
#     with ProcessPoolExecutor(max_workers=num_processes) as executor:
#         futures = {executor.submit(process_graph, i, g, dataset, method, coarse_ratios, test_idxs, processed_dir): i for i, g in enumerate(dataset)}
...
#                 print(f"An error occurred while processing graph {i}: {e}")
...
# def add_new_ratios(dataset, method, coarse_ratios, test_idxs=None, num_threads=8):
#     """Parallelize graph coarsening using ThreadPoolExecutor with a specified number of threads."""
#     processed_dir = osp.dirname(dataset.processed_paths[0])
    
#     print("Generating coarsened graphs")
#     datalist = []
    
#     # Use num_threads if specified, otherwise default to the number of CPU cores
...
class BrainDataset(InMemoryDataset):
    def __init__(
        ...
        if (split_dir / "train_idx.txt").exists() and (split_dir / "val_idx.txt").exists() and (
            ...
            assert len(self.patients) > 0, "csv files must be in " + str(split_dir)
...
def load_brain_net(
        conf: Config, graphlet_size: int, epoch: int = 0, to_graphlets=True, coarse=False, **kwargs
    ...
    if coarse:
        new_ds = get_dataset_with_coarsened_edgelist(ds, conf.coarsening_method, conf.coarse_ratios)
        ds = new_ds
...
def load_synthetic2(
    ...
    if coarse:
        new_train = get_dataset_with_coarsened_edgelist(train, conf.coarsening_method, conf.coarse_ratios)
        train = new_train
    ...
    if coarse:
        new_test = get_dataset_with_coarsened_edgelist(test, conf.coarsening_method, conf.coarse_ratios)
        test = new_test
...
def load_TUDatasets(
        conf: Config, graphlet_size: int, epoch: int = 0, to_graphlets=True, coarse=False
) -> (Dataset, Dataset, Dataset, Dict[int, Subgraph]):
    num_node_labels = {
        "PROTEINS": 3,
        "DD": 89,
    ...
    #import os
    #_TUDataset(conf.data_path, conf.dataset_name)
    #exit()
    ...
    df = None
    if conf.dataset_name in ["deezer_ego_nets", "IMDB-BINARY"]:
        from torch_geometric.transforms import OneHotDegree, Compose
        df = OneHotDegree(max_degree=100)
        #if f is not None:
            #f = compose([f, OneHotDegree(max_degree=100)])
        #else:
            #f = OneHotDegree(max_degree=100)
    ...
    #sizes = []
    #sizes_numpy = []
    #for idx, g in enumerate(ret):
    #    print(idx, g.num_nodes)
    #    sizes.append( (idx, g.num_nodes) )
    #    sizes_numpy.append(g.num_nodes)
    #sizes_numpy = np.sort(np.array(sizes_numpy))
    #q_50 = np.quantile(sizes_numpy, 0.5)
    #q_90 = np.quantile(sizes_numpy, 0.9)
    #smallest_50 = [x for x in sizes if x[1] <= q_50]
    #val_size = int(len(smallest_50)*0.1)
    #random.shuffle(smallest_50)
    #train, val = smallest_50[:-val_size], smallest_50[-val_size:]
    ##val = random.sample(smallest_50, val_size)
    #largest_10 = [x for x in sizes if x[1] >= q_90]
    #print("num graphs", len(sizes))
    #print("train", len(smallest_50))
    #print("train/val", len(train), len(val))
    #print("test", len(largest_10))
    ##def save_list(name, values):
    #    #with open(name) as f:

    #np.savetxt("train_idx.txt", np.array([x[0] for x in train], dtype=np.uint32), fmt="%d")
    #np.savetxt("val_idx.txt", np.array([x[0] for x in val], dtype=np.uint32), fmt="%d")
    #np.savetxt("test_idx.txt", np.array([x[0] for x in largest_10], dtype=np.uint32), fmt="%d")
    #print("avg train", np.array([x[1] for x in smallest_50]).mean())
    #print("avg test", np.array([x[1] for x in largest_10]).mean())
    #print(train)
    #print(val)
    #print(largest_10)
    #exit()
    ...
    if coarse:
        if conf.dataset_name == "DD": # DD has large graphs so we avoid computing the coarsened versions for the test set to save compute
            test_idxs = [5, 6, 7, 20, 21, 30, 36, 41, 54, 67, 72, 78, 89, 101, 105, 135,
                         139, 154, 158, 159, 165, 184, 185, 186, 195, 198, 203, 204, 205,
                         207, 225, 232, 238, 241, 249, 253, 260, 267, 280, 281, 286, 293,
                         298, 301, 309, 312, 316, 322, 337, 348, 349, 357, 372, 396, 408,
                         432, 440, 443, 449, 472, 475, 479, 481, 486, 491, 492, 496, 497,
                         499, 504, 505, 507, 512, 515, 516, 521, 527, 532, 552, 571, 584,
                         600, 611, 622, 625, 636, 644, 650, 657, 676, 677, 683, 686, 690,
                         737, 760, 799, 812, 814, 823, 825, 844, 857, 873, 879, 887, 888,
                         959, 962, 992, 996, 997, 1032, 1045, 1058, 1061, 1067, 1075] 
            new_ds = get_dataset_with_coarsened_edgelist(ret, conf.coarsening_method, conf.coarse_ratios, test_idxs=test_idxs)
        else:
            new_ds = get_dataset_with_coarsened_edgelist(ret, conf.coarsening_method, conf.coarse_ratios)
        ret = new_ds
    ...
    #    num_nodes_80 = g.num_coarse_nodes_80
    #    edge_index_80 = g.coarsened_edge_index_80
    #    clusters_80 = len(torch.unique(g.clusters_80))
    #    num_nodes_90 = g.num_coarse_nodes_90
    #    edge_index_90 = g.coarsened_edge_index_90
    #    clusters_90 = len(torch.unique(g.clusters_90))
    #    for name, e_i, n, c in [("og", edge_index, num_nodes, None),
    #                         ("80", edge_index_80, num_nodes_80, clusters_80),
    #                         ("90", edge_index_90, num_nodes_90, clusters_90)]:
    #        for x, y in zip(e_i[0], e_i[1]):
    #            if (x < 0 or x >= n) or (y < 0 or y >= n):
    #                print(f"Error in graph: {i}, at edge index: {name}")
    #                print("Edge_index:", e_i)
    #                print("N:", n)
    #            if c is not None:
    #                if x >= c or y >= c:
    #                    print(f"Cluster Error in graph: {i}, at edge index: {name}")
    #                    print("Edge_index:", e_i)
    #                    print("N:", n)
    #                    print("C:", c)
    ...
    if conf.model in [ModelName.GNN, ModelName.RPGNN]:
        if "SYNTHETIC3" in conf.dataset_name:
            ...
            train_ds, val_ds, test_ds, id2graphlet = load_brain_net(conf, conf.graphlet_size, to_graphlets=False,
                                                                    coarse=conf.cmd_reg, **conf.brain_params)
            return train_ds, [val_ds], [test_ds], [id2graphlet]
        elif conf.dataset_name in ["NCI1", "NCI109", "PROTEINS", "DD", "IMDB-BINARY", "deezer_ego_nets"]:
